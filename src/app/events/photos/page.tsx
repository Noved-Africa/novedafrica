  ...
  return(
    ...
    <body className='text-white font-Albert_sans '>
      ...
      <div className='mx-5 md:mt-80'>
        <p className='lg:text-5xl md:text-3xl text-center lg:ml-[15rem] md:ml-20 md:absolute md:-mt-[3.5rem] lg:mt-[19rem]'>Relive the moments, captured forever.</p>
        <p className='text-center xs:mt-10 md:mx-10 md:ml-[6rem] md:absolute md:mt-[2rem] md:mb-[1rem] lg:absolute lg:mt-[24rem] lg:ml-[24rem] lg:text-3 '>Browse through our stunning event photos and immerse yourself <br/>in the memories that make our events truly unforgettable.</p>
        <p className='w-3/12 xs:w-11/12 mx-auto md:absolute md:mt-[8rem] lg:ml-[36rem] lg:absolute lg:mt-[28rem] '><Button text='View Photos'/></p>
      </div>
...
{/* Tablet Screen */}
      <div className='lg:collapse md:mt-[13rem] w-screen bg-[#FFFADE] md:absolute md:mt-[20rem] grid xs:grid-rows-2 xs:mb-12'>
        <form className='xs:mt-5 xs:grid xs:grid-cols-2'>
          <p><input type='text' placeholder='What are you looking for?' className='rounded-[3rem] border bg-white p-2 outline-none xs:pl-12 xs:text-sm xs:w-[16rem] md:w-[25rem] h-11 xs:ml-7'/>
          <AiOutlineSearch className=' absolute xs:-mt-7 xs:ml-10' fill='grey' size={20}/></p>
          <p className='xs:w-8/12 xs:-mt-3 md:mt-0 md:ml-[6rem] xs:ml-10'><Button text = 'Search'/></p>
        </form>
        <p className='grid xs:grid-cols-4 gap-1 h-5 mx-5'>
          <Button text = 'All Photos'/>
          <Button text = 'Events'/>
      ...
      <div className='lg:visible xs:collapse  lg:mt-[2rem] bg-[#FFFADE] lg:w-screen pl-20 grid grid-cols-2'>
        ...
        <form className='grid grid-cols-2'>
          ...
          <p className='ml-[8.5rem] -mt-.5'><Button text = 'Search'/></p>
        </form>
      </div>
